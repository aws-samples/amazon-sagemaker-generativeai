    ...
    for completion, gt in zip(completions, target):
      ...
      try:
        ...
        if random.random() < 0.1:  # 1% chance to write samples into a file
          ...
          os.makedirs('/opt/ml/output/qwen/Qwen2.5-3B-Instruct/grpo/' + "completion_samples", exist_ok=True)
          log_file = os.path.join('/opt/ml/output/qwen/Qwen2.5-3B-Instruct/grpo/' + "completion_samples", "completion_samples.txt")
      ...
      try:
        ...
        if abs(float(result) - float(gt)) < 1e-5:
            ...
            if random.random() < 0.10:  # 10% chance to write fully successful samples into a file
                # todo - make the path not hardcoded
                os.makedirs('/opt/ml/output/qwen/Qwen2.5-3B-Instruct/grpo/' + "completion_samples", exist_ok=True)
                log_file = os.path.join('/opt/ml/output/qwen/Qwen2.5-3B-Instruct/grpo/' + "completion_samples", "success_completion_samples.txt")
                with open(log_file, "a") as f:
                    f.write(f"\n\n==============\n")
                    f.write(completion)
        else:
...
def grpo_function(
    ...
    print(f"Loading dataset {script_args.dataset_id_or_path}...")
