...
install_dependencies() {
    # Install Java via conda (avoids apt-get GPG issues)
    log_info "Installing OpenJDK 21 via conda for Pyserini..."
    if ! command -v javac &> /dev/null; then
        conda install -y -c conda-forge openjdk=21 2>&1 | tail -5 || log_warning "Failed to install Java via conda"
        export JAVA_HOME="${CONDA_PREFIX:-/opt/conda}"
        export PATH=$JAVA_HOME/bin:$PATH
        
        # Find and set JVM_PATH for pyjnius
        local jvm_path=$(find "${JAVA_HOME}" -name "libjvm.so" 2>/dev/null | head -1)
        if [[ -n "$jvm_path" ]]; then
            export JVM_PATH="$jvm_path"
            export LD_LIBRARY_PATH="$(dirname "$jvm_path"):${LD_LIBRARY_PATH:-}"
            log_info "Found JVM library: $jvm_path"
        fi
        
        if command -v java &> /dev/null; then
            log_info "Java installed: $(java -version 2>&1 | head -1)"
        else
            log_warning "Java installation failed - Pyserini will not work"
        fi
    else
    ...
    # Install packaging first to fix setuptools compatibility
    log_info "Installing packaging>=24.2 for setuptools compatibility..."
    uv pip install --system 'packaging>=24.2' || { log_error "Failed to install packaging"; exit 1; }
    ...
    # Pre-download Pyserini Wikipedia index to avoid timeout during training
...
" 2>&1 | tail -10 || log_warning "Failed to pre-download Pyserini index"
    log_info "Pyserini index download complete"
}
