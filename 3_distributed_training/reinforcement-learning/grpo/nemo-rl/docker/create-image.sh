...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <REPO_NAME> [TAG] [DOCKERFILE_PATH]"
    echo "Example: $0 nemo-rl-sagemaker"
    echo "Example: $0 nemo-rl-sagemaker v1.0.0"
    echo "Example: $0 nemo-rl-sagemaker latest /path/to/Dockerfile"
    exit 1
fi
...
if is_sagemaker_studio; then
    ...
    docker build --network sagemaker --platform linux/amd64 $DOCKERFILE_ARG -t ${REGISTRY}${REPO_NAME}:${TAG} .
else
    echo "Detected local/standard environment - using default network"
    docker build --platform linux/amd64 $DOCKERFILE_ARG -t ${REGISTRY}${REPO_NAME}:${TAG} .
fi
