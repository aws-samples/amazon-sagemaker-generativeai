...
from utils.reward_functions import rouge_reward_func, reward_len, format_reward_func
import wandb
...
class ScriptArguments:
    ...
    temperature: float = field(
        default=0.2, metadata={"help": "Temperature for sampling"}
    )

    top_p: float = field(default=0.9, metadata={"help": "Top-p for nucleus sampling"})
    ...
    use_vllm: Optional[bool] = field(
        default=False, metadata={"help": "Use vLLM for inference"}
    )
...
def download_model(model_name: str, local_dir: str = "/tmp/tmp_folder") -> None:
    """
    Download a model from Hugging Face Hub.

    Args:
        model_name: The model name or path to download
    ...
    try:
        ...
        logger.info(f"Model {model_name} downloaded to {local_dir}")
...
def main() -> None:
    ...
    reward_functions = [rouge_reward_func, reward_len, format_reward_func]

    # Launch training
    train(script_args, training_args, train_ds, reward_functions, test_ds)
