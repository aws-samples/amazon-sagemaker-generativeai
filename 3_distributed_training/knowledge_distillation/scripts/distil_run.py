...
def parse_arguments():
    ...
    # Dataset settings
    parser.add_argument("--dataset_name_or_path", type=str, default="", help="Dataset name")
    ...
    # Model settings
    parser.add_argument("--teacher_model_name_or_path", type=str, default="Qwen/Qwen3-4B", help="Teacher model name")
    parser.add_argument("--student_model_name_or_path", type=str, default="Qwen/Qwen3-0.6B", help="Student model name")
    ...
    # Training settings
    parser.add_argument("--hub_location", type=str, default="", help="hub location") 
    ...
    parser.add_argument("--report_to", type=str, default=None, help="report results to wandb, tensorboard")
...
def main():
    ...
    mlflow_enabled = (
        os.environ['MLFLOW_TRACKING_ARN'] is not None
        and os.environ['MLFLOW_TRACKING_ARN'] != ""
    )
    if mlflow_enabled:
        ...
        set_custom_env({"MLFLOW_EXPERIMENT_NAME": mlflow_experiment_name})
    # Load and preprocess dataset
    print(f"Loading dataset: {config['dataset']['dataset_name_or_path']}")
    if config['dataset']['dataset_name_or_path'].endswith('.json'):
        dataset = load_dataset(
            'json', data_files=config['dataset']['dataset_name_or_path'], split='train'
        )
    else:
        dataset = load_dataset(config['dataset']['dataset_name_or_path'], split=config["dataset"]["dataset_split"])
    ...
    # Load tokenizers
    print(f"Loading tokenizers for teacher ({config['models']['teacher_model_name_or_path']}) and student ({config['models']['student_model_name_or_path']})")
    teacher_tokenizer = AutoTokenizer.from_pretrained(config["models"]["teacher_model_name_or_path"])
    student_tokenizer = AutoTokenizer.from_pretrained(config["models"]["student_model_name_or_path"])
    ...
    teacher_model = AutoModelForCausalLM.from_pretrained(config["models"]["teacher_model_name_or_path"], **model_kwargs)
    student_model = AutoModelForCausalLM.from_pretrained(config["models"]["student_model_name_or_path"], **model_kwargs)
    ...
    if mlflow_enabled:
        ...
        with mlflow.start_run(run_name=os.environ.get("MLFLOW_RUN_NAME", None)) as run:
            train_dataset_mlflow = mlflow.data.from_pandas(dataset.to_pandas(), name="train_dataset")
